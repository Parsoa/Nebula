...
class ExonDepthOfCoverageCountingJob(counter.BaseExactCountingJob):
    ...
    @staticmethod
    def launch(**kwargs):
        job = DepthOfCoverageCountingJob(job_name = 'DepthOfCoverageCountingJob_', previous_job_name = "", **kwargs)
        job.execute()
...
class UniqueKmersDepthOfCoverageEstimationJob(counter.BaseExactCountingJob):
    ...
    @staticmethod
    def launch(**kwargs):
        job = UniqueKmersDepthOfCoverageEstimationJob(job_name = 'UniqueKmersDepthOfCoverageEstimationJob_', previous_job_name = 'UniqueKmersDepthOfCoverageCountingJob_', **kwargs)
        job.execute()
    ...
    def plot_reference_distribution(self, kmers):
        k = []
        for i in range(1, 11):
            k.append(list(map(lambda x: kmers[x]['count'] / i, list(filter(lambda kmer: kmers[kmer]['ref'] == i, kmers)))))
        visualizer.bar(ys = [list(map(lambda x: statistics.mean(x), k)), list(map(lambda x: statistics.std(x), k))], x = list(range(1, 11)), name = 'bar plot mean kmer coverage by reference frequency', path = self.get_current_job_directory(), x_label = 'reference frequency', y_label = 'mean coverage')
