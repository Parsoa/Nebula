...
class Deletion(StructuralVariation):
    ...
    def get_signature_kmers_with_variation(self, event, begin, end):
        ...
        # location in genome where the entire sequence ofr this sv begins
        ...
        for variant in event.variants:
            if len(variant) == 1 and variant != '-':
                ...
                if s == p:
                    continue
                #print(self.track.chrom, ':', blue(self.track.start) , '-', green(self.track.end), ' @ ', cyan(event.begin), ' -> ', white(self.sequence[:self.radius + c.ksize - offset]), cyan(variant), white(self.sequence[self.radius + c.ksize - offset + 1:]), sep = '')
