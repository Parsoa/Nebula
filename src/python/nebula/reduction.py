...
class ExtractInnerKmersJob(map_reduce.Job):
    ...
    def transform(self, track, track_name):
        ...
        #if not 'DEL@chr1_120565416_120565649' in track_name:
        if not 'INS@chr1_11031132_11031133' in track_name:
            return None
        if track.svtype == 'INV':
    ...
    def extract_inner_kmers(self, track):
        ...
        if track.svtype == 'INS':
            if hasattr(track, 'seq'):
                inner_kmers = self.extract_insertion_kmers(track, chrom)
        if track.svtype == 'DEL':
    ...
    def extract_insertion_kmers(self, track, chrom):
        c = config.Configuration()
        # kmers crossing the breakpoint on reference
        breakpoint_seq = chrom[track.begin - c.ksize: track.begin + c.ksize]
        breakpoint_kmers = {}
        assert(len(breakpoint_seq) == 64)
        for i in range(0, c.ksize + 1):
            kmer = canonicalize(breakpoint_seq[i: i + c.ksize])
            if 'N' in kmer:
                continue
            if not kmer in breakpoint_kmers:
                breakpoint_kmers[kmer] = {
                    'loci': {},
                    'tracks': {},
                    'reference': self.reference_counts_provider.get_kmer_count(kmer),
                    'trend': 'downward'
                }
            if not track.id in breakpoint_kmers[kmer]['tracks']:
                breakpoint_kmers[kmer]['tracks'][track.id] = 0
            breakpoint_kmers[kmer]['tracks'][track.id] += 1
        print(len(breakpoint_kmers), 'breakpoint kmers.')
        # inner kmers
        inner_kmers = {}
        if len(track.seq) >= 3 * c.ksize:
            inner_seq = chrom[track.begin - 250: track.begin] + track.seq.upper() + chrom[track.end: track.end + 250]
            for i in range(250 + c.ksize, len(inner_seq) - 250 - 2 * c.ksize + 1):
                kmer = canonicalize(inner_seq[i: i + c.ksize])
                if 'N' in kmer:
                    continue
                # mixed signal
                if kmer in breakpoint_kmers:
                    breakpoint_kmers.pop(kmer, None)
                    continue
...
class FilterInnerKmersJob(map_reduce.Job):
        ...
        with open(os.path.join(self.get_previous_job_directory(), track), 'r') as json_file:
            ...
            for kmer in kmers:
                ...
                if not kmer in self.kmers:
                    ...
                    self.kmers[kmer]['trend'] = kmers[kmer]['trend']
    ...
    def reduce(self):
        ...
        # makes sure no usefil loci are filtered
        for kmer in self.kmers:
